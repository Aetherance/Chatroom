...
void Client::DeleteGroup() {
  ...
  for (size_t i = 0; i < groups.size(); ++i) {
    ...
    auto delete_btn = Button("删除", [&, i] {
      msgClient_.quitGroup(msgClient_.LocalEmail(),groups[i].groupname);
    ...
    auto entry = Container::Horizontal({
        Renderer([&, i] {
          return text(" " + groups[i].groupname) | flex;
        }),
        delete_btn
    }) | border;
  ...
  auto renderer = Renderer(container, [&] {
    ...
    // 添加标题
    entries.push_back(hbox({text(" 退出群聊 ") | bold | center}) | borderDouble);
    ...
    // 空状态提示
    if (entries.size() == 1) {
      entries.push_back(text("暂无群聊") | center | dim | border);
    }
